- 新增豆包（火山引擎）语音识别作为可选转录后端，中文和各类方言（粤语、上海话、四川话等）的识别效果明显优于 Whisper
- 聚焦到视频某一时间段时，以前无论范围多小，都会把整段视频的音频送去转录，事后才裁剪到你要的区间；现在只转录聚焦的这一段（并向前后各扩展一点，避免语境被突然截断），花费大幅降低
- 支持哔哩哔哩等需要登录或有访问限制的网站
- 字幕匹配更智能，会优先选用视频本身语言的字幕，中文视频不会被误配到别的语言
- Groq 转录换成了 turbo 模型，价格从每小时 $0.111 降到 $0.04
